...
const { parseGitName, createAction, infoResponse, onExit, basicAuth, noCache, readDirEndsWithDotGit } = require('./util');
...
class Git extends EventEmitter {
  ...
  */
  ...
  dirMap(repo) {
    return path.resolve(this.repoDir, repo);
  }
  /**
   ...
   */
  list(callback) {
    callback(null, readDirEndsWithDotGit(this.repoDir).map(it => it.replace(this.repoDir, '')));
  }
  /**
